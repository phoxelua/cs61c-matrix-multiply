...
if( data_size_X % 4 == 0){ //divisible by 4 case	
	...
	//pad with zeros - GET REMAINDERS
	int new_size_X = data_size_X + 2;
	...
	for(y = 1; y < new_size_Y-1; y++){ // the y coordinate of theoutput location we're focusing on
		for(x = 1; x < new_size_X-1;x+=8){ // the x coordinate of the output location we're focusing on
						...
						//First
	          				out_temp = _mm_load_ps(out + ((x-1+4)+(y-1)*data_size_X));

						kern0 =  *(newKern + 0);
						in_temp0 =  _mm_loadu_ps(newIn + ((x-1+4) + (y-kern_cent_Y)*new_size_X));
						mult0 = _mm_mul_ps(in_temp0,kern0);
						out_temp = _mm_add_ps(out_temp, mult0);


						kern1 = *(newKern + 1);
						in_temp1 =  _mm_loadu_ps(newIn + ((x-1+1+4) + (y-kern_cent_Y)*new_size_X));
						mult1 = _mm_mul_ps(in_temp1,kern1);
						out_temp = _mm_add_ps(out_temp, mult1);

						kern2 = *(newKern  + 2);
						in_temp2 =  _mm_loadu_ps(newIn + ((x-1+2+4) + (y-kern_cent_Y)*new_size_X));
						mult2 = _mm_mul_ps(in_temp2, kern2);
						out_temp = _mm_add_ps(out_temp, mult2);
						...
						//Sec
						kern3 = *(newKern + 3);
						in_temp0 = _mm_loadu_ps(newIn + ((x-1+4) + (y-kern_cent_Y+1)*new_size_X));
						mult0 = _mm_mul_ps(kern3, in_temp0);
						out_temp = _mm_add_ps(out_temp, mult0);

						kern4 = *(newKern  + 4);
						in_temp1 =  _mm_loadu_ps(newIn + ((x-1+1+4) + (y-kern_cent_Y+1)*new_size_X));
						mult1 = _mm_mul_ps(kern4,in_temp1);
						out_temp = _mm_add_ps(out_temp, mult1);

						kern5 =  *(newKern + 5);
						in_temp2 =  _mm_loadu_ps(newIn + ((x-1+2+4) + (y-kern_cent_Y+1)*new_size_X));
						mult2 = _mm_mul_ps(kern5,in_temp2);
						out_temp = _mm_add_ps(out_temp, mult2);
						...
						//Third
						kern6 =  *(newKern + 6);	
						in_temp0 =  _mm_loadu_ps(newIn + ((x-1+4) + (y-kern_cent_Y+2)*new_size_X));
						mult0 = _mm_mul_ps(kern6,in_temp0);
						out_temp = _mm_add_ps(out_temp, mult0);


						kern7 =  *(newKern  + 7);	
						in_temp1 =  _mm_loadu_ps(newIn + ((x-1+1+4) + (y-kern_cent_Y+2)*new_size_X));
						mult1 = _mm_mul_ps(kern7,in_temp1);
						out_temp = _mm_add_ps(out_temp, mult1);

						kern8 =  *(newKern + 8);
						in_temp2 =  _mm_loadu_ps(newIn + ((x-1+2+4) + (y-kern_cent_Y+2)*new_size_X));
						mult2 = _mm_mul_ps(kern8,in_temp2);
						out_temp = _mm_add_ps(out_temp, mult2);

						_mm_storeu_ps(out+(x-1+4)+(y-1)*data_size_X, out_temp);
