...
else if (data_size_X/100 == 9 && data_size_Y/100 == 9){
	blocksize = 150;
}
...
#pragma omp parallel for private(j,i) num_threads(8)
	...
	///CONVOLUTION LOOP////
	int x,y;
	int ynsx0, ynsx1, ynsx2,ydsx,xydsx,x1,x2,x0;
...
#pragma omp parallel for  private(y,x,out_temp, kern0, kern1, kern2, kern3, kern4, kern5, kern6, kern7, kern8, mult0, mult1, mult2, in_temp0, in_temp1, in_temp2,ynsx0, ynsx1, ynsx2,ydsx,xydsx, x1,x2,x0) 		num_threads(8)
	for(y = 0; y < data_size_Y; y++){ // the y coordinate of theoutput location we're focusing on
		...
		for(x= 0; x < data_size_X/16*16;x+=16){ // the x coordinate of the output location we're focusing on
			x1 = x+1;
			x2 = x+2;
			...
			x2 = x+6;
			//First column of kernel
			out_temp = _mm_loadu_ps(out+ (x0+y*data_size_X));
			in_temp0 =  _mm_loadu_ps(newIn + x0 + ynsx0 );
			mult0 = _mm_mul_ps(in_temp0,kern0);
			out_temp = _mm_add_ps(out_temp, mult0);
			...
			//Sec column of kernel
			in_temp0 = _mm_loadu_ps(newIn + x0 + ynsx1);
			mult0 = _mm_mul_ps(kern3, in_temp0);
			out_temp = _mm_add_ps(out_temp, mult0);
			...
			//Third column of kernel
			in_temp0 =  _mm_loadu_ps(newIn + x0 + ynsx2);
			mult0 = _mm_mul_ps(kern6,in_temp0);
			out_temp = _mm_add_ps(out_temp, mult0);
			...
			_mm_storeu_ps(out + x0 + y*data_size_X, out_temp);
		...
		for(x = data_size_X/16*16; x < data_size_X/8*8;x+=8){ // the x coordinate of the output location we're focusing on
			x1 = x+1;
			x2 = x+2;
			//First column of kernel
			out_temp = _mm_loadu_ps(out+ (x+y*data_size_X));
		...
		for(x = data_size_X/8*8; x < endn;x+=4){ // the x coordinate of the output location we're focusing on
			x1 = x+1;
			x2 = x+2;
			//First column of kernel
			out_temp = _mm_loadu_ps(out+ (x+y*data_size_X));
